...
class LimpiadorAvanzadoTexto:
    ...
    def limpiar_nivel_basico(self, texto: str) -> str:
        ...
        texto_limpio = re.sub(self.patrones_limpieza['espacios_multiples'], ' ', texto_limpio)
    ...
    def limpiar_nivel_intermedio(self, texto: str) -> str:
        ...
        texto_limpio = re.sub(self.patrones_limpieza['puntuacion_multiple'], '', texto_limpio)
        
        # Eliminar caracteres especiales pero mantener apóstrofes y guiones
        texto_limpio = re.sub(self.patrones_limpieza['caracteres_especiales'], ' ', texto_limpio)
    ...
    def tokenizar_y_filtrar(self, texto: str) -> List[str]:
        ...
        for token in tokens:
            ...
            if (token.isalpha() and 
                len(token) > 2 and 
                token not in self.palabras_vacias and
                not token.isdigit()):
                tokens_filtrados.append(token)
        
        return tokens_filtrados
    
    def aplicar_normalizacion_morfologica(self, tokens: List[str], metodo: str = 'lemmatization') -> List[str]:
        """
    ...
    def proceso_limpieza_completa(self, texto: str, incluir_normalizacion: bool = True, 
                                metodo_normalizacion: str = 'lemmatization') -> Dict[str, Any]:
        """
